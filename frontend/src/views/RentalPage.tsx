...
export function RentalPage() {
  ...
  return (
    <AppShell>
      <Helmet>
        <title>Прокат - База отдыха "Победа"</title>
        <meta name="description" content={SEO_DESCRIPTIONS.rentals} />
      </Helmet>
      <div className="space-y-8 sm:space-y-12">
        ...
        <Panel className="border-[#f5d9bd] bg-[#fef3e7]">
          <div className="flex flex-col gap-3 sm:flex-row sm:gap-4">
            ...
            <div className="space-y-2">
              <h3 className="text-lg font-bold text-[#3b2a1d]">
                Правила проката
              </h3>
              <ul className="list-inside list-disc space-y-1 text-sm text-[color:var(--ink-soft)]">
                <li>
                  Инвентарь выдается при предъявлении документа, удостоверяющего
                  личность.
                </li>
                <li>
                  Оплата производится за каждый полный или неполный час
                  использования.
                </li>
                <li>
                  В случае повреждения инвентаря взимается штраф согласно
                  прейскуранту.
                </li>
                <li>
                  Возврат инвентаря осуществляется не позднее времени закрытия
                  пункта проката.
                </li>
                <li className="font-bold text-[#c96f2b]">
                  Со своими плюшками, коньками и лыжами проход бесплатный!
                </li>
              </ul>
            </div>
          </div>
        ...
        {loading ? (
          ...
          <EmptyState
            ...
            description="В данный момент прокат недоступен."
          />
        ) : (
                ...
                <div className="flex flex-1 flex-col p-5 sm:p-6">
                  <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                    ...
                    <span className="text-sm font-bold text-[#c96f2b]">
                      {item.pricePerHour
                        ? `${formatCurrency(item.pricePerHour)} / час`
                        : "Вариативная цена"}
                    </span>
                  </div>
                          ...
                          {item.priceRules.map((rule) => (
                            <tr key={rule.ruleId}>
                              <td className="px-3 py-2">
                                {rule.passengerType.toUpperCase() === "CHILD"
                                  ? "Детский"
                                  : "Взрослый"}
                              </td>
                              <td className="px-3 py-2 font-bold">
                                {formatCurrency(rule.pricePerKm)}
                              </td>
                              <td className="px-3 py-2">
                                {rule.minKm}
